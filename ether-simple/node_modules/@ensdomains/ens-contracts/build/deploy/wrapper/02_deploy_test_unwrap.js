...
const func = async function (hre) {
    ...
    let testUnwrap = await hardhat_1.ethers.getContract('TestUnwrap');
    ...
    if (!canModifyTestUnwrap) {
        ...
        testUnwrap = testUnwrap.connect(hardhat_1.ethers.provider.getSigner(contractOwner));
    }
    for (const wrapperAddress of testnetWrapperAddresses) {
        let wrapper = await hardhat_1.ethers.getContractAt('NameWrapper', wrapperAddress);
        ...
        if (!isUpgradeSet) {
            ...
            if (!canModifyWrapper && !canModifyTestUnwrap) {
                ...
                wrapper = wrapper.connect(hardhat_1.ethers.provider.getSigner(owner));
                const tx = await wrapper.setUpgradeContract(testUnwrap.address);
                console.log(`Setting upgrade contract for ${wrapperAddress} to ${testUnwrap.address} (tx: ${tx.hash})...`);
                await tx.wait();
            }
